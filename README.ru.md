...
Если финализирующая функция **не** указана:

1. запрос разбирается и находится соответствующий `Method` в файле настроек
2. выполняется [предобработка](#preprocessing--postprocessing), если она описана в секции `Enhance`
3. строится SQL запрос вызова функции на основании параметров из URL/body и с учётом соглашения о вызове
...
Если финализирующая функция **указана**:

1. запрос разбирается и находится соответствующий `Method` в файле настроек
2. строится **инициализирующий** SQL запрос вызова функции на основании параметров из URL/body и с учётом соглашения о вызове
3. выполняется **инициализирующий** запрос в БД, **получается ID новой записи**
4. выполняется [предобработка](#preprocessing--postprocessing), если она описана в секции `Enhance` **с учётом ID записи**
5. строится **финализирующий** запрос вызова функции на основании параметров из URL/body и с учётом соглашения о вызове
6. выполняется **финализирующий** запрос в БД
7. постобработка выполняется в фоне (если задана)
